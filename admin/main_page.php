...
$displayName = (strpos($email, '@') !== false) ? strstr($email, '@', true) : $email;
...
<body>
  ...
  <div class="sidebar" id="sidebar">
    <ul class="nav flex-column">
      ...
      <li class="nav-item">
        <a class="nav-link load-page" href="#" data-page="check_faculty_status.php">
          <i class="fa-solid fa-circle-info me-2"></i>Check Faculty Status
        </a>
      </li>

      <li class="nav-item">
        <a class="nav-link load-page" href="#" data-page="register.php">
          <i class="fa-solid fa-user me-2"></i>Register Faculty
        </a>
      </li>
      ...
      <li class="nav-item">
        <a class="nav-link load-page" href="#" data-page="archive.php">
  ...
  <main>
    <div class="content">
      <div class="alert alert-info" role="alert">
        <span>Welcome back, <?= htmlspecialchars($displayName) ?>! It's great to see
          you again.</span>
      </div>
      <div>
  ...
  <script>
      ...
      function setupChart(course, chartId, dateInputId) {
        ...
        function loadStats(selectedDate) {
          fetch(`fetch_statistics.php?date=${selectedDate}&course=${course}`)
            .then(res => res.json())
            .then(data => {
              const { fully_passed, conditionally_passed, failed } = data;

              messageEl.style.display = "none";
              if (chartInstance) chartInstance.destroy();

              if (fully_passed === 0 && conditionally_passed === 0 && failed === 0) {
              ...
              chartInstance = new Chart(ctx, {
                type: "pie",
                data: {
                  labels: ["Passed", "Conditionally Passed", "Failed"],
                  datasets: [{
                    data: [fully_passed, conditionally_passed, failed],
      ...
      link.addEventListener("click", function (e) {
        ...
        fetch(page)
          ...
          .then((data) => {
            ...
            } else if (page === "archive.php") {
              initUnarchiveSubjectHandler();
            }
          })
          .catch((error) => {
            Toastify({
    ...
    function initRegisterFormHandler() {
    ...
    function initArchiveSubjectBsgeHandler() {
      document.querySelectorAll('.archive-subject').forEach(btn => {
        btn.addEventListener('click', function (e) {
          e.preventDefault();
          let subject = this.dataset.subject;

          const formData = new FormData();
          formData.append("archive_subject", subject);
    ...
    function initArchiveSubjectBsabenHandler() {
      document.querySelectorAll('.archive-subject').forEach(btn => {
        btn.addEventListener('click', function (e) {
          e.preventDefault();
          let subject = this.dataset.subject;
    ...
    document.addEventListener("DOMContentLoaded", () => {
      content.addEventListener("click", function (e) {
        if (e.target.classList.contains("dropdown-item")) {
          ...
          const wrapper = e.target.closest(".status-dropdown");
          ...
          if (email) {
            ...
            formData.append("is_enabled", status);

            fetch("update_check_faculty_status.php", {
              method: "POST",
              body: formData,
            })
              .then((res) => res.text())
              .then(() => {
                ...
                btn.classList.add(status === "enabled" ? "btn-success" : "btn-danger");

                Toastify({
                  text: "Access set to " + status,
                  duration: 3000,
                  gravity: "bottom",
                  position: "right",
                  backgroundColor: "green",
    ...
    function initUnarchiveSubjectHandler() {
      document.querySelectorAll('.unarchive-subject').forEach(btn => {
        btn.addEventListener('click', function () {
          let subject = this.dataset.subject;
          let course = this.dataset.course;

          const formData = new FormData();
          formData.append("unarchive_subject", subject);
          ...
          fetch("archive.php", {
            ...
            .catch(err => {
              Toastify({
                text: "Error unarchiving subject.",
                duration: 3000,
                gravity: "bottom",
                position: "right",
                backgroundColor: "red"
